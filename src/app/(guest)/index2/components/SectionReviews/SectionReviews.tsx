...
export const SectionReviews = () => {
	return (
		<section className={block()}>
			<Heading data={{text: 'Специалисты о монетикум', tag: 'h2'}} cn={{size: 'h2', align: 'text-center', margin: 'mb-5'}}/>
			<Text data={{
				text: 'Уже на стадии разработки монетикум получил высокие отзывы со стороны психологов, воспитателей, родителей ',
				tag: 'p'
			}} cn={{size: 'text-body-1'}} className="text text-center"/>
				...
				}}>
					{
						reviews.map(item => (
							<Slider.Slide key={item.id} className={block('card')}>
								...
								<div>
									<Text className={block('text-name')}
